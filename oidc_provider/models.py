...
class RefreshToken(BaseCodeTokenModel):
    ...
    def __str__(self):
        return "{0} - {1}".format(self.client, self.refresh_token)

    @property
    def at_hash(self):
        # @@@ d-o-p only supports 256 bits (change this if that changes)
        hashed_refresh_token = (
            sha256(self.refresh_token.encode("ascii")).hexdigest().encode("ascii")
        )
        return (
            base64.urlsafe_b64encode(
                binascii.unhexlify(hashed_refresh_token[: len(hashed_refresh_token) // 2])
            )
            .rstrip(b"=")
            .decode("ascii")
        )
